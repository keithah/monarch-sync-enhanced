...
import { RETAILER_CONFIGS, Retailer } from '@root/src/shared/api/retailerApi';
...
export function ProgressIndicator({ progress }: { progress: ProgressState }) {
  ...
  const formatRetailerOrders = (lastSync: any) => {
    if (lastSync?.retailerOrders && Object.keys(lastSync.retailerOrders).length > 0) {
      const orderCounts = Object.entries(lastSync.retailerOrders)
        .map(([retailer, count]) => {
          const retailerName = RETAILER_CONFIGS[retailer as Retailer]?.name || retailer;
          return `${retailerName}: ${count}`;
        })
        .join(', ');
    ...
    return `Amazon orders: ${lastSync?.amazonOrders ?? 0}`;
  };
    ...
    <>
      {inProgress ? (
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="text-small">{formatRetailerOrders(lastSync)}</span>
...
function ProgressSpinner({ progress }: { progress: ProgressState }) {
  ...
  if (progress.phase === ProgressPhase.MonarchUpload) {
    ...
    phase = 'Downloading Retailer Orders';
    object = 'pages';
